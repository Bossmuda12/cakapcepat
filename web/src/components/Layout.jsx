...
/**
 * Menu samping dikelompokkan supaya 20 halaman tidak berjejer jadi satu daftar
 * panjang yang bikin tombol Pengaturan & kotak pengguna terdorong keluar layar.
 * Tiap grup bisa dilipat, dan pilihannya diingat di perangkat pengguna.
 */
const NAV_GROUPS = [
  {
    id: "ringkasan",
    label: "Ringkasan",
    items: [
      { to: "/dashboard", label: "Overview", end: true, icon: "M3 11.5 12 4l9 7.5M5 10v9h5v-5h4v5h5v-9" },
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/followups", label: "Follow-up", icon: "M3 11 20 3l-4 18-6-8-7-2Z" },
    ],
  },
  {
    id: "katalog",
...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* mode privat: tema tetap jalan untuk sesi ini */
    }
  }, [theme]);
  ...
  const initial = (user?.name || user?.username || user?.email || "?").trim().charAt(0).toUpperCase();
          ...
          <div className="user-box">
            <div className="user-box-row">
              <div className="user-box-avatar">
                {user?.avatar_url ? <img src={user.avatar_url} alt="" /> : <span>{initial}</span>}
              </div>
